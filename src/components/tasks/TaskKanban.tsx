...
export function TaskKanban({ launches, onRefresh }: Props) {
      ...
      {COLUMNS.map(col => {
        ...
        return (
          <div
            ...
            style={{ background: col.bg, ...(isOver ? { ringColor: col.color } : {}) }}
          ...
          >
            ...
            <div className="px-4 pt-4 pb-3 flex items-center justify-between">
              ...
              <button
                onClick={() => { setQuickCreate(col.value); setQuickTitle(""); }}
            ...
            <div className="flex-1 overflow-y-auto px-3 pb-3 space-y-2">
              {colTasks.map(task => (
                ...
                >
                  ...
                  {task.launch && (
                    <p className="text-[10px] text-slate-400 mb-2 truncate">{task.launch.nome}</p>
                  )}

                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      {task.assignee && (
                        <div className="h-5 w-5 rounded-full bg-slate-200 flex items-center justify-center text-[8px] font-bold text-slate-500" title={task.assignee.nome || ""}>
                    ...
                    <div className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${
                      task.prioridade === "urgente" ? "bg-rose-100 text-rose-600" :
                      task.prioridade === "alta" ? "bg-orange-100 text-orange-600" :
                      task.prioridade === "média" ? "bg-yellow-100 text-yellow-600" :
              ...
              {quickCreate === col.value && (
                <div className="bg-white rounded-xl p-3 shadow-sm border border-slate-200">
                  <input
                    type="text" value={quickTitle} onChange={e => setQuickTitle(e.target.value)} autoFocus
              ...
              {colTasks.length === 0 && quickCreate !== col.value && (
                <div
                  className={`h-16 rounded-xl border-2 border-dashed flex items-center justify-center transition-all ${isOver ? "border-slate-400 bg-white/40" : "border-transparent"}`}
                >
